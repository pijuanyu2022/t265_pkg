...
"""
Returns a camera matrix K from librealsense intrinsicss
"""
...
try:
    ...
    # Retreive the stream and intrinsic properties for both camerass
    ...
    # nP2 = P_right.copy()
    # nP2[0,0]=P_right[0,0]/4
    # nP2[1,1]=P_right[1,1]/4
    nP1, roi = cv2.getOptimalNewCameraMatrix(K_left, D_left, stereo_size, 0, stereo_size)
    print(roi)
    nP2, roi = cv2.getOptimalNewCameraMatrix(K_right, D_right, stereo_size, 1, stereo_size)
    # Construct Q for use with cv2.reprojectImageTo3D. Subtract max_disp from xs
    ...
    m1type = cv2.CV_32FC1
    (lm1, lm2) = cv2.fisheye.initUndistortRectifyMap(K_left, D_left, R_left, nP1, stereo_size, m1type)
    (rm1, rm2) = cv2.fisheye.initUndistortRectifyMap(K_right, D_right, R_right, nP2, stereo_size, m1type)
    ...
    while True:
        ...
        if valid:
            # Hold the mutex only long enough to copy the stereo framess
            # frame_mutex.acquire()
            ...
            color_image1 = cv2.cvtColor(center_undistorted["left"][:,max_disp:], cv2.COLOR_GRAY2RGB)
            color_image2 = cv2.cvtColor(center_undistorted["right"][:,max_disp:], cv2.COLOR_GRAY2RGB)

            if mode == "stack":
                cv2.imshow(WINDOW_TITLE, np.hstack((color_image1, color_image2)))
        key = cv2.waitKey(1)
